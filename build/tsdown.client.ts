 ...
 * Standalone copy of the loader-side helper in
...
function workspaceManifest(id: string): WorkspaceManifest {
  ...
  // Standalone layout: this repo's root manifest is the only workspace package.
  const manifest = JSON.parse(
    readFileSync(resolvePath(REPOSITORY_ROOT, 'package.json'), 'utf8'),
  ) as WorkspaceManifest
  if (manifest.name !== id) {
    throw new Error(`tsdown: ./package.json declares ${manifest.name}, not ${id}`)
  }
